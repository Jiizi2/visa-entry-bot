...
def show_fun_modal(image_path: str, title: str) -> None:
    ...
    try:
        ...
        btn = tk.Button(root, text="OK - Lanjut", command=root.destroy, font=("Arial", 12, "bold"), bg="#4CAF50", fg="white", padx=20, pady=5)
        btn.pack(pady=10)
        ...
        logger.error(f"Gagal menampilkan gambar {image_path}: {e}")
        root.destroy()
...
def main() -> None:
    ...
    try:
        ...
        # --- FUN MODE: WELCOME ---
        logger.info("🎉 Menampilkan foto WELCOME...")
        show_fun_modal("welcome.jpeg", "Welcome - Start Scanning!")

        members = [process_passport(file_path) for file_path in passport_files]
        
        # --- FUN MODE: SCAN COMPLETE ---
        logger.info("🎉 Menampilkan foto SCAN COMPLETE...")
        show_fun_modal("scan_complete.jpeg", "Scan Complete!")
            
        # --- FUN MODE: MOCK REVIEW ---
        input("\n[FUN MODE] 📝 Tekan Enter jika Anda sudah selesai melakukan review di command prompt...")
        
        logger.info("🎉 Menampilkan foto REVIEW COMPLETE...")
        show_fun_modal("review_complete.jpeg", "Review Complete!")

        write_manifest(group_id, group_dir, members)
        
        # --- FUN MODE: EXPORT COMPLETE ---
        logger.info("🎉 Menampilkan foto EXPORT COMPLETE...")
        show_fun_modal("export_complete.jpeg", "Export Complete!")
