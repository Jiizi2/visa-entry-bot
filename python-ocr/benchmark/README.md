...
graph TD
    ...
    B -->|Tulis data mentah| C[Single Pipeline Artifacts]
...
2. **Path Resolver**: Menentukan lokasi artifact pipeline tunggal secara konsisten.
...
* **Regression (PASS/FAIL)**: Memeriksa hasil terhadap snapshot baseline historis bila tersedia.
...
2. **Jalankan Benchmark Penuh**:
   ```bash
   python scripts/benchmark_dataset.py --no-resume
   ```
