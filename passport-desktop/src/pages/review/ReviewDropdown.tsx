...
export default function ReviewDropdown({ members, activeMember, resolved, onMemberSelect, reviewedMemberIds }: ReviewDropdownProps) {
  ...
  const isActiveReviewed = Boolean(activeMember?.reviewConfirmed || (reviewedMemberIds && reviewedMemberIds.has(activeMember?.id)));
  ...
  return (
    <div className="mb-4 relative z-20">
      ...
      >
        <div className={`w-2 h-2 rounded-full shrink-0 ${isActiveReviewed ? 'bg-green-500' : 'bg-amber-400'}`}></div>
      ...
      {dropdownOpen && (
        <div className="absolute top-full mt-1 left-0 w-full bg-white border border-slate-300 rounded-lg shadow-lg py-1 flex flex-col z-30 max-h-64 overflow-y-auto">
          {members.map((m: any) => {
            ...
            const isReviewed = Boolean(m.reviewConfirmed || (reviewedMemberIds && reviewedMemberIds.has(m.id)));
            return (
              <button 
                key={m.id}
              ...
              >
                <div className={`w-2 h-2 rounded-full shrink-0 ${isReviewed ? 'bg-green-500' : 'bg-amber-400'}`}></div>
                <div className="flex flex-col flex-1 overflow-hidden">
                  <span className={`text-[14px] leading-[20px] font-semibold tracking-wider whitespace-nowrap overflow-hidden text-ellipsis ${isActive ? 'text-slate-900 font-bold' : 'text-slate-700'}`}>
                    {memberDisplayName(m)}
                  </span>
                  <span className="text-[11px] leading-tight text-slate-600">
