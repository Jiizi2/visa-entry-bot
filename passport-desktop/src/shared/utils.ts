...
export function formatRecentStamp(value) {
...
export function formatProgressValue(value) {
  const numeric = Number(value);
...
export function nestedStringValue(node, path) {
  const value = nestedValue(node, path);
  return typeof value === "string" ? value : "";
}

export function nestedNumberValue(node, path) {
  const value = nestedValue(node, path);
  return typeof value === "number" ? value : null;
}

export function nestedValue(node, path) {
  let current = node;
...
export function valueByPath(node, path) {
...
export function setValueByPath(node, path, nextValue) {
  const parts = pathParts(path);
...
export function dateValueForInput(value) {
  return normalizeDateToNusuk(String(value ?? "").trim());
}

export function normalizeDateToNusuk(value) {
...
export function toNusukDate(yearValue, monthValue, dayValue) {
...
export function monthNameToNumber(value) {
