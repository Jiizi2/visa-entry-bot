...
import { memberReviewStatus } from "../entry/entry.js";
import {
...
export function renderFieldReviewRows({ state, member, members }: any) {
  const resolved = ensureResolvedProfile(member);
  ...
  const visibleFields = pair.categoryIds
    ...
    .filter(Boolean)
    .flatMap((category: any) => REVIEW_FIELDS.filter(([key]) => category.keys.includes(key)));
...
export function renderCompanionReviewPanel({ state, member, members }: any) {
...
export function renderFieldCategoryTabs({ dom, state, member }) {
  ...
  dom.fieldCategoryTabs.innerHTML = FIELD_CATEGORY_PAIRS.map((pair) => {
    const categories = pair.categoryIds
      .map((categoryId) => FIELD_CATEGORY_DEFS.find((item) => item.id === categoryId))
      .filter(Boolean);
    const keys = categories.flatMap((category: any) => category.keys);
