import { basenameFromPath, formatDurationMs } from "../../shared/utils.js";
import { memberScanTotalMs } from "../../shared/metrics.js";
import { memberDisplayName } from "../../shared/members.js";
import {
  passportCropApplied,
...
export function createPassportPreviewController({
  ...
  async function ensureImageForMember(member: any) {
  ...
  function applyImageResult(memberId: string, result: any) {
  ...
  function setZoom(nextZoom: number, options: any = {}) {
  ...
  function applyZoom(options: any = {}) {
  ...
  function restoreScrollCenter(centerRatio: any) {
    const frame = dom.passportPreviewFrame;
    if (!frame) {
      return;
    }
...
export function clampPassportPreviewZoom(value: any) {
