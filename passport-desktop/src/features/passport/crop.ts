...
} from "../prepared/preview.js";
import {
  basenameFromPath,
} from "../../shared/utils.js";
import type { CropRect } from "../../shared/types.js";
...
export function passportCropModeDescriptor(mode: any) {
  if (mode === "prepared") {
    return {
...
function numberOr(value: any, fallback: any) {
  const numeric = Number(value);
  return Number.isFinite(numeric) ? numeric : fallback;
}

export function clampPassportCropZoom(value: any) {
  const numeric = numberOr(value, 1);
  return Math.round(Math.min(2, Math.max(0.75, numeric)) * 100) / 100;
}

export function normalizeCropRect(rect: any, imageWidth: any, imageHeight: any) {
...
export function defaultPassportCropRect(imageWidth: any, imageHeight: any) {
...
export function createPassportCropController({
  ...
  requestFrame,
  activeMember,
  activePreparedItem = () => null as any,
  loadPassportImageData,
  loadPreparedImageData = null as any,
  saveCroppedPassportImage,
  savePreparedPassportImage = null as any,
  applyPreparedSession = (session?: any, activeId?: any) => {},
  ...
  async function loadSourceImageForMember(member: any) {
    const candidates: string[] = passportCropSourceImageCandidates(member);
  ...
  async function loadSourceImageForPreparedItem(item: any) {
  ...
  function renderCropDataUrl(rect: any) {
  ...
  function imageFrameForCanvas(canvas: any, sourceImage: HTMLImageElement, zoom: number) {
    const padding = 18;
  ...
  function drawCropOverlay(context: CanvasRenderingContext2D) {
  ...
  function canvasRectFromCrop(rect: any) {
    if (!imageFrame) return { x: 0, y: 0, width: 0, height: 0 };
    return {
  ...
  function canvasPointFromEvent(event: any) {
    const canvas = dom.passportCropCanvas;
  ...
  function imagePointFromCanvas(point: any, options: any = {}) {
    if (!imageFrame || !image) {
      return null;
    }
  ...
  function hitTestCropHandle(point: any) {
  ...
  function isPointInsideCrop(point: any) {
    const rect = canvasRectFromCrop(cropRect);
    return point.x >= rect.x && point.x <= rect.x + rect.width && point.y >= rect.y && point.y <= rect.y + rect.height;
  }

  function updateCanvasCursor(point: any) {
    const canvas = dom.passportCropCanvas;
    if (!canvas) {
      return;
    }
...
export function cropHandlePoints(rect: any) {
  const x1 = rect.x;
  const y1 = rect.y;
...
export function updateCropRectForInteraction(mode: any, startRect: any, dx: any, dy: any, imageWidth: any, imageHeight: any) {
